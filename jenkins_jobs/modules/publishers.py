...
def artifactory(parser, xml_parent, data):
    ...
    :arg bool publish-build-info: Push build metadata with artifacts (default False)
    ...
    XML.SubElement(artifactory, 'deployArtifacts').text = str(data.get('deploy-artifacts', True)).lower()
    XML.SubElement(artifactory, 'discardOldBuilds').text = str(data.get('discard-old-builds', False)).lower()
    XML.SubElement(artifactory, 'discardBuildArtifacts').text = str(data.get('discard-build-artifacts', False)).lower()
    XML.SubElement(artifactory, 'deployBuildInfo').text = str(data.get('publish-build-info', False)).lower()
    XML.SubElement(artifactory, 'includeEnvVars').text = str(data.get('include-env-vars', False)).lower()
    XML.SubElement(artifactory, 'evenIfUnstable').text = str(data.get('even-if-unstable', False)).lower()
    XML.SubElement(artifactory, 'runChecks').text = str(data.get('run-checks', False)).lower()
    XML.SubElement(artifactory, 'includePublishArtifacts').text = str(data.get('include-publish-artifacts', False)).lower()
    XML.SubElement(artifactory, 'passIdentifiedDownstream').text = str(data.get('pass-identified-downstream', False)).lower()
    XML.SubElement(artifactory, 'licenseAutoDiscovery').text = str(data.get('license-auto-discovery', True)).lower()
    XML.SubElement(artifactory, 'disableLicenseAutoDiscovery').text = str(data.get('disable-license-auto-discovery', False)).lower()
    XML.SubElement(artifactory, 'enableIssueTrackerIntegration').text = str(data.get('enable-issue-tracker-integration', False)).lower()
    XML.SubElement(artifactory, 'aggregateBuildIssues').text = str(data.get('aggregate-build-issues', False)).lower()
    XML.SubElement(artifactory, 'allowPromotionOfNonStagedBuilds').text = str(data.get('allow-promotion-of-non-staged-builds', False)).lower()
    XML.SubElement(artifactory, 'blackDuckRunChecks').text = str(data.get('black-duck-run-checks', False)).lower()
    XML.SubElement(artifactory, 'blackDuckIncludePublishedArtifacts').text = str(data.get('black-duck-include-published-artifacts', False)).lower()
    XML.SubElement(artifactory, 'autoCreateMissingComponentRequests').text = str(data.get('auto-create-missing-component-requests', True)).lower()
    XML.SubElement(artifactory, 'autoDiscardStaleComponentRequests').text = str(data.get('auto-discard-stale-component-requests', True)).lower()
    XML.SubElement(artifactory, 'filterExcludedArtifactsFromBuild').text = str(data.get('filter-excluded-artifacts-from-build', False)).lower()
    ...
    XML.SubElement(details, 'snapshotsRepositoryKey').text = data['snapshot-repo-key']
    ...
    XML.SubElement(deployment_patterns, 'includePatterns').text = data.get('include-deployment-pattern', '')
    XML.SubElement(deployment_patterns, 'excludePatterns').text = data.get('exclude-deployment-pattern', '')

    deployment_patterns = XML.SubElement(artifactory, 'envVarsPatterns')
    XML.SubElement(deployment_patterns, 'includePatterns').text = data.get('include-env-var-pattern', '')
    XML.SubElement(deployment_patterns, 'excludePatterns').text = data.get('exclude-env-var-pattern', '*password*,*secret*')
